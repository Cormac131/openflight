...
class SelectorParams:
    """Bounds on the selector: window width, per-frame motion, patience and SNR gate."""
...
@dataclass
class SelectorState:
    """Track carried between frames: last bin, Q8 velocity, misses and liveness."""
    ...
    def copy(self) -> SelectorState:
        """Independent copy, so a rejected frame can leave the caller's state untouched."""
        return SelectorState(self.selected_bin, self.velocity_q8, self.misses, self.active)


@dataclass(frozen=True)
class SelectorResult:
    """One frame's decision: candidates, the proposed window and its confidence."""
...
def select_window(
    ...
    if chosen is None:
        ...
        if state.active:
            state.velocity_q8 = int((state.velocity_q8 + (chosen - before_bin) * 256) / 2)
        else:
            state.velocity_q8 = 0
