...
| **`horizontal_deg` has no on-chip path yet** (`_tx2_horizontal_proxy` and its helpers are unported -- see `firmware/iwr6843/solve/solve_lcmf.h`'s banner and Task 6's report). It is production-consumed: `runtime.py:445-450` calls `estimate_lcmf_v1(capture.raw, ...)` with the full raw capture bytes and reads `horizontal_deg`/`horizontal_raw_deg` off the result, and `server.py` references `horizontal_deg` 147 times downstream. Until `_tx2_horizontal_proxy` is ported to the DSS, `runtime.py` must keep transferring the full raw capture off-chip to compute the horizontal estimate -- so **this project's stated Goal ("a normal shot transfers results rather than raw IQ cells") cannot be met by porting `lcmf`'s vertical decision alone**. This is not a surprise to be found during integration: a separate task must port `_tx2_horizontal_proxy` (or `runtime.py` must be changed to accept a vertical-only on-chip result plus a still-transferred raw capture for the horizontal axis, which gives up part of the transfer savings) before the "handful of floats" goal is actually reached. |
