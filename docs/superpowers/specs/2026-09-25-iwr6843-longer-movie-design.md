...
- Reducing shot-to-shot turnaround. Turnaround is capped, not improved
  (see Budgets).
...
- **Turnaround cap: 8.0 s.** At 1,041,667 baud (`l3_dump.c:4233`, ~104 kB/s),
  51 frames = 778,464 B = **7.47 s** shot-to-shot, up from 6.59 s today.
  Exceeding 8.0 s requires an explicit re-decision, including if the bank
  probe succeeds (60 frames would be 8.79 s).
...
| 4/16 | Turnaround | Capped at 8.0 s and stated here. |
...
| 15 | Host timeout | Raise `monitor.py:599` to cover the 64-frame ceiling, plus a test asserting every shipped profile leaves a stated margin. Sweep the read path for other timeouts. |
...
3. Shot-to-shot turnaround stays at or under 8.0 s.
4. `uv run pytest tests/ -v` passes and
   `uv run pylint src/openflight/ --fail-under=9` holds.
5. The firmware builds via `make -C firmware docker-build`.
