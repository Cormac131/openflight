...
- **Configurable IWR6843 capture compression.** One firmware image can now
  ...
  profile and an advanced 36-frame, 2 ms, 53-bin IQ8 profile. Both retain the
  ...
  preview and a lower-priority asynchronous IQ8 packer, allowing HWA rearm to
  preempt conversion before the next 2 ms frame. Firmware `stats` exposes
  missed frames, pack waits and overruns, scratch state, and clipped components.
